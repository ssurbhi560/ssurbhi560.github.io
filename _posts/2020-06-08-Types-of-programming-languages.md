...
date: 2020-6-8
---
There is no boundation to the number of ways we can classify programming languages. Here I will be trying to focus on particulary one of those classification which is related to the way of declaration of the constructs in the language.

Let's start with discussing about **statically typed language** and **dynamically typed language**.
...
Here the memory allocated for the variable does not know about the type of the variable until the code is run. It stores the value at some random memory location and then binds the variable to that memory container, making the value of that container accessible through that variable name. So declaration of the data type does not matter here as it will get to know the type of the value at run time.
